...
async def handle_admin_coupon_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    user = update.effective_user
    if not is_admin(user.id):
        return False
    sess = ensure_session(user.id)
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if await handle_admin_coupon_text(update, context):
        return
    if not await guard_bot_online(update, context):
        return
    user = update.effective_user
